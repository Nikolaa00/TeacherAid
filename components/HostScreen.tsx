...
export function HostScreen({ code, hostKey, initialSim, resetOnLoad, motion }: Props) {
  ...
  return (
    <div ref={stage} className="min-h-dvh flex flex-col p-[clamp(1.5rem,4vw,4rem)] pb-[clamp(1rem,2.5vw,2.5rem)] select-none">
      ...
      {!state ? (
        <div className="flex-1 grid place-items-center text-ink-2 text-2xl">Connecting…</div>
      ) : state.phase === "lobby" ? (
      ...
      {state?.phase === "lobby" && (
        <p className="mt-6 text-ink-3 text-base" data-anim>
      ...
      <div className="text-right leading-tight">
        {s && (
          <>
            ...
            <p className="text-ink-2 text-[1.125rem] mt-1">Today: {s.topic}</p>
          </>
        )}
...
function Lobby({ state, joinUrl, shortUrl, qrSize }: { state: SessionState; joinUrl: string; shortUrl: string; qrSize: number }) {
  return (
    <section className="flex-1 grid grid-cols-[1.1fr_auto] gap-[clamp(2rem,5vw,6rem)] items-center">
      <div>
        <h1 className="font-display font-semibold text-[clamp(2.75rem,5.6vw,5.25rem)] leading-[0.98] tracking-tight max-w-[14ch]" data-anim>
          Scan to join today&rsquo;s class.
        </h1>
        <p className="mt-5 text-[clamp(1.5rem,2.3vw,2.125rem)] text-ink-2 break-all" data-anim>
          {shortUrl || " "}
        </p>
        <div className="mt-[clamp(2rem,6vh,4rem)] flex items-end gap-5" data-anim>
          <CountUp value={state.totals.joined} className="font-display font-semibold text-[clamp(6rem,13vw,11rem)] leading-[0.85]" />
          <span className="text-[clamp(1.5rem,2.4vw,2.25rem)] text-ink-2 pb-2">joined</span>
        </div>
        <ul className="mt-6 flex flex-wrap gap-2.5 max-h-[22vh] overflow-hidden" aria-label="Joined">
          {state.players.map((p) => (
            <li key={p.nick} className="bg-paper-2 text-ink px-3.5 py-1.5 rounded-full text-[1.25rem] font-display">
              {p.nick}
            </li>
          ))}
        </ul>
...
function QuestionLabel({ state }: { state: SessionState }) {
  ...
  return (
    <p className="text-ink-2 text-[1.25rem]" data-anim>
      <span className="text-ink font-semibold">{q.type === "prediction" ? "Opener" : "Pop-up"}</span> · {state.qIndex + 1} of{" "}
      {state.hasNext ? state.qIndex + 2 : state.qIndex + 1}
      <span className="text-ink-3"> · {state.session.generatedLabel}: {state.session.topic}</span>
    </p>
  );
}
    ...
    <section className="flex-1 flex flex-col">
      <div className="flex items-start justify-between gap-8">
        <div className="flex-1">
          ...
          <h2 className="mt-3 font-display font-semibold text-[clamp(2rem,4vw,4rem)] leading-[1.04] tracking-tight max-w-[24ch]" data-anim>
            {q.prompt}
          </h2>
        </div>
        {joinUrl && (
          <div className="shrink-0 text-center" data-anim>
            ...
            <p className="text-ink-3 text-sm mt-1">late? scan</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-auto pt-5 flex items-end gap-4" data-anim>
        <CountUp value={state.answered} className="font-display font-semibold text-[clamp(2.5rem,5vw,4.5rem)] leading-none" />
        <span className="text-ink-2 text-[clamp(1.25rem,2vw,1.75rem)] pb-1">of {state.totals.joined} answered</span>
      </div>
    </section>
  );
}
  ...
  return (
    <section className="flex-1 min-h-0 grid grid-cols-[1.15fr_0.85fr] gap-[clamp(2rem,5vw,5rem)]">
      <div className="min-w-0">
        <QuestionLabel state={state} />
        <h2 className="mt-3 font-display font-semibold text-[clamp(1.75rem,3vw,3rem)] leading-[1.06] tracking-tight max-w-[26ch]" data-anim>
          {q.prompt}
        </h2>
        <div className="mt-6" data-anim>
          <VoteBars options={q.options} counts={state.counts} revealed correct={q.correct} size="compact" />
        </div>
        <p className="mt-6 text-[clamp(1.125rem,1.8vw,1.5rem)] leading-snug max-w-[44ch]" data-anim>
          {q.reveal}
        </p>
        <p className="mt-2 text-ink-2 text-[clamp(1rem,1.4vw,1.25rem)]" data-anim>
          +20 for a reason × {state.totals.reasons}
          {fastest.length > 0 && <> · speed bonus: {fastest.slice(0, 3).join(", ")}</>}
        </p>
      </div>
      <div className="min-w-0 min-h-0">
        <p className="text-ink-2 text-[1.25rem] mb-1" data-anim>
          Why, in their words
        </p>
        <ul className="overflow-hidden max-h-[66vh]">
          {state.reasons.slice(0, 8).map((r) => (
            <li key={r.nick} className="border-t border-paper-3 py-2.5" data-anim>
              <p className="font-display text-[clamp(1.125rem,1.7vw,1.5rem)] leading-snug">&ldquo;{r.reason}&rdquo;</p>
              <p className="text-ink-2 mt-0.5 text-[1rem]">
                {r.nick} · {LETTERS[r.choice]}
              </p>
            </li>
          ))}
          {state.reasons.length === 0 && (
            <li className="border-t border-paper-3 py-3 text-ink-3 text-[1.25rem]" data-anim>
  ...
  return (
    <section className="flex-1 flex flex-col">
      ...
      <ol className="mt-4 max-w-[60rem]">
        {rows.map((r) => {
          ...
          return (
            <li
              ...
              data-anim
            >
              <span className={`font-display font-semibold text-[clamp(1.75rem,3.2vw,3rem)] leading-none ${top ? "text-amber-2" : "text-ink-3"}`}>
                {r.rank}
              </span>
              <span className="font-display font-semibold text-[clamp(1.75rem,3.2vw,3rem)] leading-none truncate">{r.nick}</span>
              <span className={`font-display tabular text-[clamp(1.75rem,3.2vw,3rem)] leading-none ${top ? "text-amber-2" : ""}`}>
                {r.points}
              </span>
            </li>
          );
        ...
        {rows.length === 0 && (
          <li className="py-4 border-t border-paper-3 text-ink-3 text-[1.5rem]" data-anim>
            Nobody joined this round.
          </li>
        )}
      </ol>
      <p className="mt-auto pt-6 text-ink-2 text-[1.25rem]" data-anim>
      ...
      <dl className="mt-[clamp(1rem,4vh,3rem)] grid grid-cols-4 gap-8 max-w-[70rem]">
        {stats.map(([n, label]) => (
          <div key={label} data-anim>
            <dd className="font-display font-semibold tabular text-[clamp(4rem,9vw,8rem)] leading-[0.9]">{n}</dd>
            <dt className="text-ink-2 text-[1.5rem] mt-3">{label}</dt>
          </div>
        ))}
      </dl>
      <p className="mt-auto pt-8 text-[1.5rem] leading-snug max-w-[48ch]" data-anim>
